  ...
  return {
    ...
    upsert: async (args: any) => {
      ...
      if (modelName === 'subscription') {
        ...
        const existing = state.users.find((u: any) => u.id === args?.where?.id);
        if (existing) {
          Object.assign(existing, updateData);
          record = existing;
        } else {
          const newUser = {
            id: args?.where?.id || (crypto.randomUUID ? crypto.randomUUID() : 'mock-new-user-id'),
            createdAt: new Date().toISOString(),
            ...createData,
          };
        ...
        record = {
          id: crypto.randomUUID ? crypto.randomUUID() : 'mock-profile-id',
          userId: args?.where?.userId || 'mock-uuid',
          ...createData,
          ...updateData
        };
      }
...
export function createMockPrismaProxy(realPrisma: PrismaClient) {
      ...
      if (prop === '$transaction') {
        return async (callback: any) => {
          if (typeof callback === 'function') {
            return await callback(createMockPrismaProxy(realPrisma));
          }
          return callback;
        };
      }
